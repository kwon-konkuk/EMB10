...
#define DRIVER_CLASS "MyModuleClass_button"
...
static ssize_t driver_read(struct file *FILE, char *user_buffer, size_t count, loff_t *offs)
{
    ...
    /* Read value of button and Select button */
    if (gpio_get_value(5) == 1)
    {
        tmp = gpio_get_value(5) + '0';
        delta = 1;
    }
    else if (gpio_get_value(6) == 1)
    {
 ...
 */
static int __init ModuleInit_button(void)
{
    printk("Hello, Kernel, button\n");
 ...
 */
static void __exit ModuleExit_button(void)
{
    gpio_free(5);
    gpio_free(6);
    cdev_del(&my_device);
    ...
    class_destroy(my_class);
    unregister_chrdev_region(my_device_nr, 1);
    printk("Goodbye, Kernel, button\n");
}

module_init(ModuleInit_button);
module_exit(ModuleExit_button);
