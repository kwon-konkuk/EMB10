...
#define DRIVER_CLASS "MyModuleClass_segment"
...
/**
 * @brief This function is called, when th module is loaded into the kernel
 */
static int __init ModuleInit_segment(void)
{
    printk("Hello, Kernel, segment\n");
    ...
    /* Set D1 ~ D4 segments GPIO*/
    /* GPIO 2 init */
    if (gpio_request(2, "rpi-gpio-4"))
    {
 ...
 */
static void __exit ModuleExit_segment(void)
{
    gpio_set_value(2, 0);
    gpio_set_value(3, 0);
    gpio_set_value(4, 0);
    ...
    class_destroy(my_class);
    unregister_chrdev_region(my_device_nr, 1);
    printk("Goodbye, Kernel, segment\n");
}

module_init(ModuleInit_segment);
module_exit(ModuleExit_segment);
